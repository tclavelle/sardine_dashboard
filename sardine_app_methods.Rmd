...
source('functions/sardine_optim.R')
...
### Fishing Mortality
Fishing mortality for *S. lemuru* is estimated using the mean length ($L_{Bar}$) method described in Ault *et al.* (2005), which requires the minimum, maximum, and average length reported in the catch, together with estimates of the Von Bertalanffy growth parameter K, natural mortality, and first length at full selectivity ($L_C$). 

```{r}
# Calculate LBar
lBar <- length_df %>%
  filter(length >= 14.75) %>%
  mutate(total_lengths = length * freq) %>%
  summarize(lbar = sum(total_lengths) / sum(freq)) %>% .$lbar

# Total Mortality
Z <- vbK * (Linf - lBar) / (lBar - 14.75)

# Fishing Mortality
f <- Z - M
```

Where $L_C = 14.75$, $L\infty = 22.1$, $M = 1.786$, and $\bar{L}=15.17$. The resulting estimate of total mortality ($Z = 21.35$) is excessively high and unreliable. 
